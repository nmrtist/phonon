...
Phonon is a single-crate Rust (edition 2024) desktop application for molecular
and materials modeling. It ships two front-ends — an egui/wgpu GUI and a
headless CLI — that are both driven by the same `.psh` scripting language defined
...
`main.rs` dispatches on its arguments: no args → launch the GUI; a `.psh` path →
