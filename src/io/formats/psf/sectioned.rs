...
pub(crate) fn parse_extension_block(
    ...
    while *index < lines.len() {
        ...
        if !line.starts_with('#') {
            bail!("PSF @<PHONON>{name} must use section tags like #KEY or #TABLE");
        }
    ...
    bail!("PSF @<PHONON>{name} block is empty")
}
...
fn parse_sectioned_block(
    ...
    while *index < lines.len() {
        ...
        let tag = PsfSectionTag::parse(line)
            .ok_or_else(|| anyhow!("PSF @<PHONON>{name} has unsupported section tag {line}"))?;
        *index += 1;
    ...
    if sections.is_empty() {
        bail!("PSF @<PHONON>{name} block contains no sections");
    }
...
fn parse_table_section(name: &str, lines: &[&str], index: &mut usize) -> Result<PsfTableBlock> {
    let Some(columns_line) = next_section_data_line(lines, index) else {
        bail!("PSF @<PHONON>{name} #TABLE is missing columns");
    };
    let columns = columns_line
        ...
        .collect::<Vec<_>>();
    if columns.is_empty() {
        bail!("PSF @<PHONON>{name} #TABLE has no columns");
    }
    ...
    while *index < lines.len() {
        ...
        *index += 1;
        ...
        if fields.len() != columns.len() {
            bail!(
                "PSF @<PHONON>{name} #TABLE row has {} fields but header defines {} columns",
                fields.len(),
                columns.len()
            );
        }
