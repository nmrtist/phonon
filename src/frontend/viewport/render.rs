...
pub(super) fn atom_visible(
    ...
    let style = visual_state.resolved_atom_style(structure, atom_index);
    if style == AtomStyle::Hidden {
        return false;
    }
    if atom_drawn_as_cartoon(structure, visual_state, atom_index) {
        return false;
    }
    // A Cartoon style on a non-polymer atom has no backbone to draw.
    if style == AtomStyle::Cartoon {
        return false;
    }
...
/// Whether an atom is rendered through the cartoon path: it must be styled
/// [`AtomStyle::Cartoon`] and belong to a standard amino-acid residue of a
/// biopolymer that covers the whole structure.
pub(super) fn atom_drawn_as_cartoon(
    structure: &Structure,
    visual_state: &ViewportVisualState,
    atom_index: usize,
) -> bool {
    if visual_state.resolved_atom_style(structure, atom_index) != AtomStyle::Cartoon {
        return false;
    }
    let Some(biopolymer) = usable_biopolymer(structure) else {
        return false;
    };
    let Some(Some(residue_index)) = biopolymer.residue_for_atom.get(atom_index) else {
        return false;
    };
    biopolymer
        .residues
        .get(*residue_index)
        .is_some_and(|residue| residue.is_standard_amino_acid)
}

/// Whether any atom is currently drawn as cartoon, used to decide whether to
/// build the cartoon (and dependent surface) scene at all.
