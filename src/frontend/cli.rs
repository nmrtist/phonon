...
pub fn parse_cli_script_request(args: &[String]) -> Result<Option<CliScriptRequest>> {
    ...
    if script_path
        ...
        .is_none_or(|value| !value.eq_ignore_ascii_case("psh"))
    {
        bail!("CLI mode expects a `.psh` script path as the first argument");
    }
...
pub fn cli_help_text() -> &'static str {
    "Phonon\n\nUsage:\n  phonon                         Launch the GUI\n  phonon <script.psh> [--name value | --name=value | --flag]\n\nScript variables:\n  Use ${name} for required parameters\n  Use ${name:-default} for optional parameters with defaults\n\nExamples:\n  phonon examples/protein_plotting/cartoon_mesh_surface_cli.psh --input examples/protein_plotting/1MXE.pdb --output out/protein.png\n  phonon examples/protein_plotting/cartoon_mesh_surface_cli.psh --input examples/protein_plotting/1MXE.pdb --output out/protein.png --width 1200 --height 900\n"
}
...
mod tests {
    ...
    fn parses_script_request_with_named_args() {
        ...
        assert_eq!(request.script_path.to_string_lossy(), "examples/demo.psh");
