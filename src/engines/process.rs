...
mod tests {
    ...
    fn echo_streams_stdout_and_exits() {
        ...
        let working_dir = std::env::temp_dir().join("phonon_process_echo_test");
    ...
    fn timeout_kills_long_running_process() {
        ...
        let working_dir = std::env::temp_dir().join("phonon_process_timeout_test");
    ...
    fn stdin_is_delivered_to_child() {
        ...
        let working_dir = std::env::temp_dir().join("phonon_process_stdin_test");
        let config = ProcessConfig::new(executable, working_dir)
            .args(args)
            .stdin_lines(&["SOL", "0"]);
        let result = run(config).expect("subprocess runs to completion");
    ...
    fn stdin_writer_does_not_deadlock_when_child_ignores_it() {
        ...
        let working_dir = std::env::temp_dir().join("phonon_process_stdin_deadlock_test");
