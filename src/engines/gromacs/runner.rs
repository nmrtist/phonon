...
";
    ...
    fn wsl_gromacs_energy_minimization_runs_end_to_end() {
        let working_dir = std::env::temp_dir().join("phonon_gmx_em_integration");
    ...
    fn wsl_gromacs_full_md_pipeline_runs_end_to_end() {
        ...
        let working_dir = std::env::temp_dir().join("phonon_gmx_full_md_integration");
        let _ = fs::remove_dir_all(&working_dir);
    ...
    fn wsl_gromacs_generated_topology_runs_full_md() {
        ...
        let working_dir = std::env::temp_dir().join("phonon_gmx_generated_top_integration");
        let _ = fs::remove_dir_all(&working_dir);
