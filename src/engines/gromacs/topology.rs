...
mod tests {
    ...
    fn inline_topology_is_written_verbatim() {
        let target = std::env::temp_dir().join("phonon_topology_inline_test");
    ...
    fn file_topology_is_copied() {
        let target = std::env::temp_dir().join("phonon_topology_file_test");
        let source_path = std::env::temp_dir().join("phonon_topology_file_test_source.top");
        let _ = fs::remove_dir_all(&target);
