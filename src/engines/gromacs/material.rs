...
fn ions_mdp(periodic_molecules: bool) -> String {
    let mut mdp = String::from(
        "; Phonon-generated parameters for genion preprocessing\n\
         integrator    = steep\n\
         nsteps        = 0\n\
...
mod tests {
    ...
    fn wsl_gromacs_framework_flexible_minimizes() {
        let working_dir = std::env::temp_dir().join("phonon_gmx_framework_flexible");
    ...
    fn wsl_gromacs_framework_rigid_minimizes() {
        let working_dir = std::env::temp_dir().join("phonon_gmx_framework_rigid");
        let _ = std::fs::remove_dir_all(&working_dir);
    ...
    fn wsl_gromacs_framework_solvated_minimizes() {
        let working_dir = std::env::temp_dir().join("phonon_gmx_framework_solvated");
        let _ = std::fs::remove_dir_all(&working_dir);
    ...
    fn wsl_gromacs_framework_custom_force_field_minimizes() {
        let working_dir = std::env::temp_dir().join("phonon_gmx_framework_custom");
        let _ = std::fs::remove_dir_all(&working_dir);
        let build_dir = working_dir.join("build");
        let run_dir = working_dir.join("run");
