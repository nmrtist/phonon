...
//!   Phonon project and its format version can be checked *before* opening any
//!   database;
...
/// Top-level descriptor written at the project root (not inside `.phonon/`) so
/// the folder is recognizable and version-checkable at a glance.
pub const MANIFEST_FILE: &str = "phonon.project";
const LOCK_FILE: &str = "session.lock";
const MAINTENANCE_LOG_FILE: &str = "maintenance.log";

const DO_NOT_EDIT: &str = "# Phonon-generated file — safe to read, do not edit.";
...
pub fn write_manifest(project: &ProjectSession) -> Result<()> {
    ...
    let contents = format!(
        "{DO_NOT_EDIT}\n\
         phonon_project_format {PROJECT_FORMAT_VERSION}\n\
...
pub fn check_manifest_compatibility(project: &ProjectSession) -> Result<()> {
    ...
    if let Some(format) = parse_manifest_field(&contents, "phonon_project_format")
        && format > PROJECT_FORMAT_VERSION
    {
        bail!(
            "project was created with a newer Phonon format (v{format}); this build supports up to v{PROJECT_FORMAT_VERSION}"
        );
    }
    Ok(())
}
...
pub fn acquire_lock(project: &ProjectSession) -> bool {
    let path = project.phonon_dir.join(LOCK_FILE);
...
pub fn release_lock(project: &ProjectSession) {
    let _ = fs::remove_file(project.phonon_dir.join(LOCK_FILE));
}
...
fn append_maintenance_log(project: &ProjectSession, message: &str) {
    let path = project.phonon_dir.join(MAINTENANCE_LOG_FILE);
...
mod tests {
    ...
    fn session(name: &str) -> ProjectSession {
        ...
        fs::create_dir_all(root.join(".phonon")).unwrap();
    ...
    fn manifest_roundtrips_and_rejects_newer_formats() {
        ...
        fs::write(&path, "phonon_project_format 9999\n").unwrap();
