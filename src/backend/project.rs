...
pub const PHONON_DIR: &str = ".phonon";
...
impl ProjectSession {
    pub fn from_root(root: PathBuf, name: String) -> Self {
        let phonon_dir = root.join(PHONON_DIR);
        Self {
            project_db: phonon_dir.join(PROJECT_DB),
            compounds_db: phonon_dir.join(COMPOUNDS_DB),
            phonon_dir,
            root,
            name,
        }
...
pub fn is_valid_project_dir(path: &Path) -> bool {
    path.join(PHONON_DIR).is_dir() && path.join(PHONON_DIR).join(PROJECT_DB).is_file()
}
...
pub fn create_project(root: &Path, name: &str) -> Result<ProjectSession> {
    ...
    std::fs::create_dir_all(project_root.join(PHONON_DIR))
...
pub fn open_project(path: &Path) -> Result<(ProjectSession, ProjectSnapshot)> {
    if !is_valid_project_dir(path) {
        bail!("not a valid Phonon project");
    }
    let fallback_name = path
        .file_name()
        .and_then(|name| name.to_str())
        .unwrap_or("Phonon Project")
        .to_string();
